...
import {
  ...
  HelpCircle, Shuffle, ChevronRight, CheckCircle2, AlertCircle, LogOut, Loader2, Languages, Upload
} from 'lucide-react';
...
export default function StudentPage() {
  ...
  // Handle uploading multiple local audio files
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files) return;
    const files = Array.from(e.target.files);
    const studentName = label || token || 'Student';

    const newClips: RecordedClip[] = files.map(file => {
      const url = URL.createObjectURL(file);
      const fileNameWithoutExt = file.name.substring(0, file.name.lastIndexOf('.')) || file.name;
      const dateTime = getFormattedDateTime();
      const clipName = `${studentName} - ${dateTime} - ${fileNameWithoutExt}`;
          ...
          <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col items-center justify-center relative overflow-hidden shadow-2xl">
            ...
            <div className="flex justify-center items-center gap-8 mt-2">
              {!isRecording && (
                ...
                >
                  <div className="w-16 h-16 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center group-hover:bg-slate-800 group-hover:border-slate-700 text-slate-400 group-hover:text-slate-100 transition shadow-lg">
                  ...
                  <input
                    ...
                    multiple
                    accept="audio/*,audio/wav,audio/mp3,audio/mpeg,audio/ogg,audio/m4a,audio/x-m4a,audio/webm,audio/aac,audio/flac,audio/3gpp"
              ...
              <div className="flex flex-col items-center gap-2">
                {!isRecording ? (
                  <button
                    ...
                    className="w-20 h-20 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center hover:bg-emerald-500 hover:scale-105 active:scale-95 transition-all shadow-lg hover:shadow-emerald-500/20 duration-300 disabled:opacity-50 cursor-pointer"
                    title={t.student.startRecord}
                  >
                    <Mic className="w-8 h-8 text-emerald-400 hover:text-slate-950 transition duration-300" />
                  </button>
                ) : (
                  <button
                    onClick={stopRecording}
                    className="w-20 h-20 rounded-full bg-rose-500/20 border border-rose-500/40 flex items-center justify-center hover:bg-rose-500 hover:scale-105 active:scale-95 transition-all animate-pulse shadow-lg hover:shadow-rose-500/20 duration-300 cursor-pointer"
                ...
                <span className="text-xs font-bold text-slate-400">
                  {isRecording ? (lang === 'en' ? 'Stop' : 'Dừng') : (lang === 'en' ? 'Record' : 'Ghi âm')}
                </span>
              </div>
            </div>
          </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`w-9 h-9 rounded-xl flex items-center justify-center border transition ${activePlayingId === clip.id
