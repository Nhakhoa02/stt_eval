...
import { Mic, Volume2, ShieldCheck, ChevronRight, Lock, KeyRound, Sparkles, Languages } from 'lucide-react';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (savedToken && savedRole) {
      setSession({
        role: savedRole,
        label: savedLabel || (savedLang === 'en' ? 'Member' : 'Thành viên')
      });
    }
  }, []);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between p-6 relative overflow-hidden font-sans">
      ...
      <header className="max-w-5xl w-full mx-auto flex justify-between items-center z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={toggleLang}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900/60 hover:bg-slate-900 border border-slate-850 hover:border-slate-800 rounded-xl text-xs font-bold text-slate-350 hover:text-slate-200 transition cursor-pointer"
          >
      ...
      <main className="max-w-md w-full mx-auto flex flex-col items-center justify-center gap-8 py-20 z-10">
        ...
        <div className="flex flex-col items-center text-center gap-4">
          <div className="w-16 h-16 bg-gradient-to-tr from-indigo-500/10 to-emerald-500/10 border border-slate-880 rounded-3xl flex items-center justify-center shadow-2xl relative">
          ...
          {session ? (
            <div className="flex flex-col gap-4 text-center">
              <div className="bg-emerald-500/5 border border-emerald-500/15 p-3 rounded-2xl">
                <p className="text-xs text-slate-400">{t.home.welcomeBack}</p>
              ...
              <button 
                ...
                className="text-[11px] text-slate-500 hover:text-slate-350 transition underline underline-offset-4 mt-1 cursor-pointer"
              >
                {t.home.logoutCurrent}
              </button>
            </div>
          ) : (
            <div className="flex flex-col gap-4">
              <div className="flex gap-3 items-start bg-slate-950/60 p-4 border border-slate-950 rounded-2xl">
                ...
                <div className="text-left">
                  <h4 className="text-xs font-bold text-slate-200">{t.home.authRequired}</h4>
                  <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
                    {t.home.authInstructions}
                  </p>
                </div>
              </div>
              ...
              <Link
                href="/admin"
                className="w-full py-3 bg-slate-900 border border-slate-800 text-slate-350 hover:text-slate-100 hover:bg-slate-800 font-bold rounded-2xl flex items-center justify-center gap-1.5 transition text-xs"
              >
                {t.home.adminLogin}
              </Link>
            </div>
      ...
      <footer className="max-w-5xl w-full mx-auto text-center py-4 z-10">
        <p className="text-[10px] text-slate-650">{t.home.footer}</p>
      </footer>
    </div>
  );
}
